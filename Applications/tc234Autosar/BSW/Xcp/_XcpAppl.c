...
unsigned int GetBaseAddr(void) /* PRQA S 5013 */ /* MD_Xcp_5013 */
{
  HMODULE hModule;
  HANDLE  hProcess;
  MODULEINFO moduleInfo;
  char strDLLPath[256]; /* PRQA S 5013 */ /* MD_Xcp_5013 */

  GetModuleFileName((HINSTANCE)&__ImageBase, strDLLPath, 255);
  hProcess = GetCurrentProcess();
  hModule = GetModuleHandle(strDLLPath);
  GetModuleInformation(hProcess, hModule, &moduleInfo, sizeof(MODULEINFO));
  return (unsigned int)(moduleInfo.lpBaseOfDll); /* PRQA S 5013 */ /* MD_Xcp_5013 */
}
# undef CONST
