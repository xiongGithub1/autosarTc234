...
IFX_LOCAL_INLINE void Spi_lHwRouteQspiSrcToDma(uint32 Module);
IFX_LOCAL_INLINE void Spi_lDbgSnapshotSrcDmaLink(void);
...
volatile uint32 g_SpiDbgDmaRuntimeEnterCnt;
volatile uint32 g_SpiDbgAfterEnableRxHwCnt;
volatile uint32 g_SpiDbgAfterEnableTxHwCnt;
volatile uint32 g_SpiDbgAfterSetQspiTxIntrCnt;
volatile uint32 g_SpiDbgMainFuncErrCheckCnt;
volatile uint32 g_SpiDbgBusHandlerEnterCnt;
volatile uint32 g_SpiDbgLastTxIntStatus;
volatile uint32 g_SpiDbgLastRxIntStatus;
volatile uint32 g_SpiDbgLastDmaErrStatus;
volatile uint32 g_SpiDbgLastQspiStatus;
volatile uint32 g_SpiDbgLastDmaTxChannel;
volatile uint32 g_SpiDbgLastDmaRxChannel;
volatile uint32 g_SpiDbgLastDmaTransferCount;
volatile uint32 g_SpiDbgLastDmaTxTransferCount;
volatile uint32 g_SpiDbgLastIsDmaTxReqd;
volatile uint32 g_SpiDbgLastPackedVar;
volatile uint32 g_SpiDbgLastModule;
volatile uint32 g_SpiDbgSrcQspi1Tx;
volatile uint32 g_SpiDbgSrcQspi1Rx;
volatile uint32 g_SpiDbgSrcDmaCh2;
volatile uint32 g_SpiDbgSrcDmaCh3;
volatile uint32 g_SpiDbgDmaTsr2;
volatile uint32 g_SpiDbgDmaTsr3;
...
static void Spi_lBusHandler(uint32 Module, uint8 ApiAccessId)
{
  ...
  g_SpiDbgBusHandlerEnterCnt++;
  g_SpiDbgLastModule = Module;
  g_SpiDbgLastQspiStatus = SPI_HW_MODULE[Module].STATUS.U;
...
)
{
  ...
  IsLastChannel = 0U;
  g_SpiDbgMainFuncErrCheckCnt++;
  g_SpiDbgLastDmaErrStatus = ErrorStatus;
  g_SpiDbgLastModule = Module;
  g_SpiDbgLastDmaTxChannel = (uint32)DmaTxChannel;
  g_SpiDbgLastDmaRxChannel = (uint32)DmaRxChannel;
  g_SpiDbgLastQspiStatus = SPI_HW_MODULE[Module].STATUS.U;
  if (Module == (uint32)SPI_QSPI1_INDEX)
  {
    Spi_lDbgSnapshotSrcDmaLink();
  }
  ...
  if (SPI_BUSY == Spi_lGetModAsyncBusStatus((uint8)Module))
  {
    ...
    {  /* No Error/s */
      ...
      RxIntStatus = (uint32)SPI_SFR_RUNTIME_MCAL_DMAGETTRANSFERINTSTATUS\
                                                                (DmaRxChannel);
      g_SpiDbgLastTxIntStatus = TxIntStatus;
      g_SpiDbgLastRxIntStatus = RxIntStatus;
      g_SpiDbgLastQspiStatus = SPI_HW_MODULE[Module].STATUS.U;
...
IFX_LOCAL_INLINE void Spi_lHwRouteQspiSrcToDma(uint32 Module)
{
  ...
  #if((SPI_SAFETY_ENABLE == STD_ON) && (SPI_QM_ASYNC_MASTER_MODULES_USED == 0U))
  TxDmaChannel = DMA_CHANNEL_INVALID;
  RxDmaChannel = DMA_CHANNEL_INVALID;
  #endif

  #if (SPI_SAFETY_ENABLE == STD_ON)
  if(SPI_ASIL_MASTER == Spi_lGetModuleKind((uint8)Module))
  {
    TxDmaChannel = Spi_AsilChnlAccess[0U].DmaChannelIdx.TxDmaChannel;
    RxDmaChannel = Spi_AsilChnlAccess[0U].DmaChannelIdx.RxDmaChannel;
  }
  else
  {
    #if (SPI_QM_ASYNC_MASTER_MODULES_USED != 0U)
    TxDmaChannel = Spi_QmChnlAccess[\
                  Spi_lGetQmModuleVarIndex(Module)].DmaChannelIdx.TxDmaChannel;
    RxDmaChannel = Spi_QmChnlAccess[\
  ...
  TxRouteMask = ((uint32)TxDmaChannel & SPI_SRC_QSPI_SRPN_MSK) | \
                                                              SPI_SRC_QSPI_TOS_DMA_MSK;
  RxRouteMask = ((uint32)RxDmaChannel & SPI_SRC_QSPI_SRPN_MSK) | \
                                                              SPI_SRC_QSPI_TOS_DMA_MSK;

  {
    const uint32 txSre = MODULE_SRC.QSPI.QSPI[Module].TX.U & SPI_SRC_QSPI_TX_SETMASK2;
    const uint32 rxSre = MODULE_SRC.QSPI.QSPI[Module].RX.U & SPI_SRC_QSPI_RX_SETMASK3;

    MODULE_SRC.QSPI.QSPI[Module].TX.U = \
    (MODULE_SRC.QSPI.QSPI[Module].TX.U & (~SPI_SRC_QSPI_ROUTE_CLEARMASK)) | \
    TxRouteMask;
    MODULE_SRC.QSPI.QSPI[Module].RX.U = \
    (MODULE_SRC.QSPI.QSPI[Module].RX.U & (~SPI_SRC_QSPI_ROUTE_CLEARMASK)) | \
    RxRouteMask;
  }
}
...
IFX_LOCAL_INLINE void Spi_lHwEnableQspiTxIntr(uint32 Module)
{
  /* Route QSPI TX service request to the configured DMA channel. */
  Spi_lHwRouteQspiSrcToDma(Module);
...
IFX_LOCAL_INLINE void Spi_lDbgSnapshotSrcDmaLink(void)
{
  ...
  g_SpiDbgDmaTsr2 = DMA_TSR002.U;
  g_SpiDbgDmaTsr3 = DMA_TSR003.U;
}

IFX_LOCAL_INLINE void Spi_lHwSetQspiTxIntr(uint32 Module)
{
  /* Ensure QSPI TX is routed to DMA before software trigger (SETR). */
  Spi_lHwRouteQspiSrcToDma(Module);

  /* Re-enable SRE after route (route clears SRE). */
  SPI_SFR_RUNTIME_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].TX.U,\
                            SPI_SRC_QSPI_TX_CLEARMASK,SPI_SRC_QSPI_TX_SETMASK1)
  SPI_SFR_RUNTIME_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].TX.U,\
                            SPI_SRC_QSPI_TX_CLEARMASK,SPI_SRC_QSPI_TX_SETMASK2)

  /* Route also clears RX SRE; restore so RX DMA can accept QSPI RX requests. */
  SPI_SFR_RUNTIME_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].RX.U,\
...
IFX_LOCAL_INLINE void Spi_lHwEnableQspiRxIntr(uint32 Module, uint8 ApiAccessId)
{
  /* Route QSPI RX service request to the configured DMA channel. */
  Spi_lHwRouteQspiSrcToDma(Module);
...
IFX_LOCAL_INLINE void Spi_lHwInitIntr(uint32 Module)
{
  ...
  /* Route after flag clears so TOS/SRPN are not wiped by CLEARMASK4. */
  Spi_lHwRouteQspiSrcToDma(Module);

  /* Re-enable RX service request after route (route clears SRE). */
  SPI_SFR_RUNTIME_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].RX.U,\
                      SPI_SRC_QSPI_RX_CLEARMASK3,SPI_SRC_QSPI_RX_SETMASK3)
}
...
)
{
  ...
  g_SpiDbgDmaRuntimeEnterCnt++;
  g_SpiDbgLastPackedVar = PackedVar;
  g_SpiDbgLastDmaTransferCount = DmaTransferCount;
...
*/
  ...
  g_SpiDbgLastDmaTxTransferCount = DmaTxTransferCount;
  g_SpiDbgLastIsDmaTxReqd = isDmaTxReqd;
  ...
  /* Enable RX channel interrupt trigger (ICH) for polling completion detect. */
  SPI_SFR_RUNTIME_MCAL_DMAENABLECHINTRTRIGGER(DmaRxChannel);
...
*/
  ...
  /* Clear Pending Interrupt */
  Spi_lHwDmaClearIntr(DmaRxChannel);
  /* Enable RX channel interrupt trigger (ICH) for completion detect. */
  Mcal_DmaEnableChIntrTrigger(DmaRxChannel);
