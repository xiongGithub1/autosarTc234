...
FUNC(void, ECUM_CODE) EcuM_StartupTwo(void)
{
...
#endif
  {
...
#endif
    {
      ...
      /* #33 Initialize ComM before BswM action lists use ComM_CommunicationAllowed(). */
      ComM_Init();

      /* #34 Initialization of the BswM. */
      BswM_Init( ECUM_BSWM_CONFIG_POINTER); /* SBSW_ECUM_FUNCTIONCALL_POINTER */

#if(ECUM_FIXED_BEHAVIOR == STD_ON)
      EcuM_SetModuleState(ECUM_STATE_STARTUP_TWO);

      /* #35 Initialize BSW that does not need NvM Data (DriverInitListTwo). */
      ECUM_DRIVERINITLIST_TWO(); /* SBSW_ECUM_FUNCTIONPOINTER_CALL */

      /* #36 Execute user-specific code prior the Rte is started (EcuM_OnRTEStartup()). */
      EcuM_OnRTEStartup();

# if(ECUM_SUPPORT_RTE == STD_ON)
      /* #37 Start the AUTOSAR Rte. */
      (void) Rte_Start();
# endif
      /* #38 Initialize BSW that needs NvM Data (DriverInitListThree).*/
      ECUM_DRIVERINITLIST_THREE(); /* SBSW_ECUM_FUNCTIONPOINTER_CALL */

#else
      /* #39 Only relevant for EcuM Flex: From this point the EcuM is in the UP phase. */
      EcuM_SetModuleState(ECUM_STATE_RUN);
#endif

      /* #40 Check if there are wakeups stored in the EcuM_BswM_BufferedWakeups variable and in that case notify them to BswM. */
      EcuM_EnterCriticalArea_0();
