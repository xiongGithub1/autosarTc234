...
))
{
  ...
  /* #30 Restore the next PCXI and the link register */
  Os_Hal_ContextIntRestore(Next);                                                                                       /* SBSW_OS_HAL_FC_CALLER */
}
...
))
{
  ...
  /* #40 Initialize the program status word with call depth 1 due to the reserved CSA frame */
  ...
  *(uint32*)(PCXIAddr + OS_HAL_CSA_SP_OFFSET) = CurrentConfig->StackEndAddr;                                            /* PRQA S 0306 */ /* MD_Os_Hal_0306 */ /* SBSW_OS_HAL_PWA_PCXI */
  ...
  /* #60 Initialize the return address */
  *(uint32*)(PCXIAddr + OS_HAL_CSA_RA_OFFSET) = CurrentConfig->ReturnAddress;                                           /* PRQA S 0306 */ /* MD_Os_Hal_0306 */ /* SBSW_OS_HAL_PWA_PCXI */
  ...
  /* #80 Initialize the MPU stack settings */
  if(Os_MpIsEnabled() != 0)
  {
  ...
  /* #90 Initialize the runtime context variable */
  Context->PreviousContextInfo = Os_Hal_Mfcr(OS_HAL_PCXI_OFFSET);                                                       /* SBSW_OS_HAL_PWA_CALLER */
