...
))                                                                                                                      /* COV_OS_HALPLATFORMPFRTUNSUPPORTED_XX */
{
  ...
  if(OS_UNLIKELY(
      Os_CounterIsFutureValueInternal(Timer->MaxCountingValue, Timer->MaxDifferentialValue, compare, now) == 0))
  {
    ...
    else
    {
      ...
      /* #100 Loop while compare value is in the past or now with respect to the last read counter value. */
               ...
               Timer->MaxCountingValue, Timer->MaxDifferentialValue, compareValue, timeAfterSetCompare) == 0));
    }
  }
}
 ...
 *  Os_TimerPfrtGetDyn()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE P2VAR(Os_TimerPfrtType, AUTOMATIC, OS_VAR_NOINIT), OS_CODE,                /* PRQA S 3219 */ /* MD_Os_3219 */
OS_ALWAYS_INLINE, Os_TimerPfrtGetDyn,
(
  P2CONST(Os_TimerPfrtConfigType, AUTOMATIC, OS_CONST) Timer
))                                                                                                                      /* COV_OS_HALPLATFORMPFRTUNSUPPORTED_XX */
{
  return (P2VAR(Os_TimerPfrtType, AUTOMATIC, OS_VAR_NOINIT))((Timer->SwCounter).Dyn);                                   /* PRQA S 0310, 3305 */ /* MD_Os_0310, MD_Os_3305 */
}
...
)
{
  ...
  Os_TimerPfrtSetNextHardwareCompareValue(Timer);                                                                       /* set the initial compare value. */ /* SBSW_OS_FC_PRECONDITION */
}
...
ISR(Os_TimerPfrtIsr)                                                                                                    /* COV_OS_HALPLATFORMPFRTUNSUPPORTED_XX */
{
  ...
  pfrtTimer = (P2CONST(Os_TimerPfrtConfigType, AUTOMATIC, OS_CONST)) me->Counter;                                       /* PRQA S 0310 */ /* MD_Os_0310 */
