...
*******************************************************************************/
/* defined AdcSWGroup due to the Symbolic Name change btw AS403 and AS321 */
#ifndef AdcSWGroup
...
#define ADC_START_SEC_VAR_FAST_32BIT
#include "MemMap.h"
...
#define ADC_STOP_SEC_VAR_FAST_32BIT
#include "MemMap.h"
...
void Adc_SWGroupDemo(void)
{
  ...
  print_f("\nResult Access mode  : Streaming, Linear\n ");
  print_f("\nStreaming Length    : 5                \n ");
  print_f("\nChannels in a groups: AN1/(AN6-TC22x) - 8bit resolution   \n"); 
  print_f("                        AN0/(AN5-TC22x) - 10 bit resolution \n ");
  /*Conversion is stopped automatically when once configured
  ...
  */
  print_f("\n\n ");
  /* Adc_Init not called here, since it is called from EcuM */
  ...
  /* Initialise the Result buffer for AdcSWGroup */
  ...
  if(AdcBufferSetupStatus == E_OK)
  {
    ...
    /* Initialise the no of conversted samples to 0 before starting the 
       conversion
    */
    Demo_AdcNoSamp = 0;

    /* Initialise the no. of group samples (Streaming length)
    ...
    */
    ...
    print_f("AN1/(AN6-TC22x) : %4x\n",*(Demo_ResultPtr));
    print_f("AN0/(AN5-TC22x) : %4x\n",*(Demo_ResultPtr + AdcSWGroupNoSamples));
