...
#define CSA_START_TC0   0x70004000              /* start address of CSA tc0 */
#endif
...
#ifndef ISTACK_TC0
#define ISTACK_TC0      8k                      /* interrupt stack size tc0 */
#endif
...
derivative tc27x
{
        ...
        memory dspr0 // Data Scratch Pad Ram
        {
                ...
                size = 184k;
                type = ram;
                map shared (dest=bus:tc0:fpi_bus, dest_offset=0xd0000000, size=184k, priority=2, exec_priority=0);
                map non_shared (dest=bus:sri, dest_offset=0x70000000, size=184k);
        }
        ...
        memory pspr0 // Program Scratch Pad Ram
        {
                ...
                size = 8k;
                type = ram;
                map shared (dest=bus:tc0:fpi_bus, dest_offset=0xc0000000, size=8k, exec_priority=2);
                map non_shared (dest=bus:sri, dest_offset=0x70100000, size=8k);
        }
...
#                               include "traptab0.lsl"           
        ...
        section_layout :tc0:linear
        {
                group bmhd0 (run_addr = 0xa0000000, ordered) reserved "bmi_header_0_protection" (size=0x20,alloc_allowed=absolute);
                group bmhd1 (run_addr = 0xa0020000, ordered) reserved "bmi_header_1_protection" (size=0x20,alloc_allowed=absolute);
                group bmhd2 (run_addr = 0xa000ffe0, ordered) reserved "bmi_header_2_protection" (size=0x20,alloc_allowed=absolute);
                group bmhd3 (run_addr = 0xa001ffe0, ordered) reserved "bmi_header_3_protection" (size=0x20,alloc_allowed=absolute); 
                "_A9_DATA_TC1" := 0;
                "_LITERAL_DATA_TC1" := 0;
                "_A8_DATA_TC1" := 0;
                "_SMALL_DATA_TC1" := 0;
